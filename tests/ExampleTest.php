...
it('loads the package config', function () {
    expect(config('filament-project-passport.navigation.group'))->toBe('Developer Support')
        ...
        ->and(config('filament-project-passport.authorization.restricted_to_admins'))->toBeTrue()
        ->and(config('filament-project-passport.authorization.restrict_non_production'))->toBeFalse();
});
